<?php  include 'koneksi.php'; ?>
 
<html> 
    <head>
        <title>UAS PEMROGRAMAN WEB</title>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
    </head>
    <body> 
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link" href="index.php">Beranda</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="pasien.php">Pasien</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" aria-current="page" href="dokter.php">Dokter</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="kamar.php">Kamar</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="upload.php">File</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="login.php">Admin</a>
            </li>
        </ul>
        <br>
        <h3 align="center">Edit Data Kamar</h3>
        <br>
        <?php
        $id = $_GET['id'];
        $query = "SELECT * FROM tb_kamar WHERE id_kamar='$id'";
        $data = mysqli_query($koneksi, $query);
        $d = mysqli_fetch_array($data);
        ?>
        <form action="kamar_edit_proses.php" method="POST">
            <table align="center">
                <tr>
                    <td>Nama Kamar</td>
                    <td><input type="text" name="nama_kamar" value="<?= $d['nama_kamar'] ?>"></td>
                </tr>
                <tr>
                    <td>Kapasitas</td>
                    <td><input type="number" name="kapasitas" min="1" max="7" value="<?= $d['kapasitas'] ?>"></td>
                </tr>
                <tr>
                    <td></td>
                    <td>
                        <input type="submit" value="Simpan">
                        <input type="hidden" name="id_kamar" value="<?= $d['id_kamar'] ?>">
                    </td>
                </tr>
            </table>
        </form>
    </body>
</html>